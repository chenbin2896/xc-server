...
public enum AuthCode implements ResultCode {
    ...
    AUTH_LOGIN_APPLYTOKEN_FAIL(false, 23007, "token校验异常！"),
    AUTH_LOGIN_TOKEN_SAVEFAIL(false, 23008, "token保存异常！"),
    ...
    //操作代码
    @Schema(name = "操作是否成功", example = "true", requiredMode = Schema.RequiredMode.REQUIRED)
    boolean success;
    //操作代码
    @Schema(name = "操作代码", example = "22001", requiredMode = Schema.RequiredMode.REQUIRED)
    int code;
    //提示信息
    @Schema(name = "操作提示", example = "操作过于频繁！", requiredMode = Schema.RequiredMode.REQUIRED)
    String message;
